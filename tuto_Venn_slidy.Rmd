...
```{r, fig.height = 7, fig.width = 7, echo = F, warning = F}
...
grid.draw(venn.diagram(list_No, filename = NULL,
             ...
             cat.dist = c(0.2, 0.2, 0.2, 0.2, 0.25),
...
<Br><Br>
<center><font size = "7">Compromis avec le package `VennDetail`</center></font> 
<center><font size = "5">(Bioconductor)</center></font>
<Br><Br>
...
## Objet de class `venn` {.build .smaller}
...
<Br><Br>
<center><font size = "7">Customisation avec le package `VennDiagram`</center></font>
<Br><Br>
...
## Diagramme de venn avec le package `VennDiagram`
 ...
 - Exporte les graphs en .tiff, .svg, .png
 - Récupération des listes des sous-ensembles pas immédiate

## Diagramme de venn avec le package `VennDiagram`

De part ses nombreux paramètres graphiques, le diagramme produit par défaut est très simple. 
Il faut donc mettre un peu les mains dedans pour le rendre attractif
...
<font size = "5">`futile.logger::flog.threshold(futile.logger::ERROR, name = "VennDiagramLogger")`</font>


## Diagramme par défaut

Prends directement en entrée les listes
...
## Avec la palette de couleurs "tully"
...
## Extraction des listes (fonction maison)

 - Extraction des effectifs des sous-ensembles
...
## créé une liste avec le détail de chaque sous-ensemble dans une liste
list_result <- vector("list", length(levels(result$Subset)))
for (i in seq(length(list_result)))
{
  tmp <- as.character(result[result$Subset == levels(result$Subset)[i],2])
  list_result[[i]] <- affil[affil$Cluster %in% tmp,]
  names(list_result)[i] <- levels(result$Subset)[i]
}

## export dans un fichier Excel, 1 feuille par liste
library(openxlsx)
write.xlsx(list_result, file = "list_result.xlsx", colNames = T)
```

<center>![](C:/P/GIT/tuto_Venn/logo_excel.png)</center>
